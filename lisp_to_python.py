...
class LispToPythonInterpreter:
    ...
    def interpret(self, lisp_code: str) -> str:
        ...
        if parser.pos < len(tokens):
            raise SyntaxError(
                f"Unexpected tokens after expression: {[t.value for t in tokens[parser.pos:]]}"
            )
...
def main():
    ...
    parser.add_argument(
        "input_file", help="Input Lisp file (.lisp extension recommended)"
    )
    parser.add_argument(
        "output_file", help="Output Python file (.py extension recommended)"
    )
    ...
    try:
        ...
        print(
            f"Converted {len(results)} expressions from {input_file} to {output_file}"
        )
